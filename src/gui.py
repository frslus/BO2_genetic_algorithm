...
class GUI:
    ...
    def __init__(self):
        ...
        # # algorithm handling
        # self.alg_menu = tk.Menu(self.menu_bar, tearoff=0)
        # self.alg_menu.add_command(label="----")
        # self.menu_bar.add_cascade(menu=self.alg_menu, label="Algorytm")

        # visual settings
        self.set_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.set_menu.add_command(label="Czcionka +", command=self.increase_font)
        self.set_menu.add_command(label="Czcionka -", command=self.decrease_font)
        self.menu_bar.add_command(label="Wyrównaj okno", command=self.normalize_window_size)
        self.menu_bar.add_cascade(menu=self.set_menu, label="Ustawienia")

        self.root.config(menu=self.menu_bar)

        #button
        self.button = tk.Button(self.root,text="Wygeneruj rozwiązanie", font=("Helvetica", 16), command = self.generate_solution)
